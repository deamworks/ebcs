...
def update_submission(submission_id):
    """
    แอดมินแก้ไขใบยื่น — ทำได้เฉพาะสถานะ draft/pending_payment เท่านั้น
    (ยื่นแล้วจ่ายเงินแล้ว (paid) ห้ามแก้ เพราะมีใบเสร็จรับเงินอ้างอิงยอดเดิมไปแล้ว)
    Request: PUT /api/admin/submissions/<id>
    ทุกการแก้ไขบันทึกลง audit_logs อัตโนมัติ

    [FIX] เดิมแก้ได้แค่ยอดรวมระดับ submissions ตรงๆ (ใช้ verify/แก้ยอดผิดเล็กน้อย)
    ตอนนี้รองรับแก้แบบเดียวกับที่ผู้ประกอบการกรอกเองได้ทั้งหมด — ถ้าส่ง "licenses"/
    "other_incomes" มาด้วย จะลบใบอนุญาต/รายได้อื่นเดิมแล้วสร้างใหม่ทั้งชุด (เหมือน
    PUT /api/operator/submissions/<id> ของผู้ประกอบการ) พร้อมคำนวณยอดกองทุนใหม่เอง
    ฝั่ง backend (ไม่เชื่อยอดที่ frontend ส่งมาตรงๆ)
    """
            ...
            if old["status"] == "paid":
                return jsonify({
                    ...
                    "error": {"code": "ALREADY_PAID",
                              "message": "ใบยื่นแบบนี้ชำระเงินแล้ว ไม่สามารถแก้ไขได้"}
                }), 400

            has_full_detail = "licenses" in data or "other_incomes" in data

            if has_full_detail:
                # แก้ทั้งชุด (รายได้/ค่าลดหย่อนต่อใบอนุญาต + รายได้อื่น) เหมือนหน้า
                # กรอกแบบของผู้ประกอบการ — คำนวณยอดกองทุนใหม่เองฝั่ง backend
                from .operator import calculate_fund, new_uuid  # lazy import กัน circular import
                ...
                calculation  = calculate_fund(total_income, deduction_amount, old["due_date"])

                cur.execute("""
                    UPDATE submissions SET
                        total_income            = %s,
                        total_income_financial  = %s,
                        deduction_amount         = %s,
                        fund_amount              = %s,
                        vat_amount               = %s,
                        extra_amount             = %s,
                    ...
                    WHERE id = %s
                """, (
                    total_income, total_income_financial, deduction_amount,
                ...
                changes = {
                    "total_income":    {"old": str(old.get("total_income")),    "new": str(total_income)},
                    "net_amount":      {"old": str(old.get("net_amount")),      "new": str(calculation["net_amount"])},
                }

            else:
                # ── แก้แค่ยอดรวมตรงๆ (ไม่ได้ส่งรายละเอียดต่อใบอนุญาตมา) ─────────
                updatable = [
