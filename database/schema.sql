-- ════════════════════════════════════════════════════════
-- NBTC Filing Database Schema (MySQL 8.0)
-- รันอัตโนมัติตอน MySQL สร้าง database ครั้งแรก (ลบ volumes/db เพื่อรันใหม่)
--
-- ไฟล์นี้เป็นแหล่งอ้างอิงเดียว (single source of truth) ของโครงสร้างฐานข้อมูล
-- รวมทุกการเปลี่ยนแปลงที่เคยแยกเป็น migration ไว้ในไฟล์นี้ทั้งหมดแล้ว
-- (ไม่มีโฟลเดอร์ migrations/ แยกต่างหากอีกต่อไป) — ถ้าจะแก้โครงสร้างฐานข้อมูล
-- ให้แก้ไขไฟล์นี้ตรงๆ แล้วรันคำสั่ง ALTER ที่จำเป็นด้วยมือกับฐานข้อมูลที่มีอยู่แล้ว
-- ════════════════════════════════════════════════════════
